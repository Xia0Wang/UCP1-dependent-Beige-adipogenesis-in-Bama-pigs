...
setwd("E:/BIIF/liutianxia/Adipocyte_scRNA_seq_8/3_结果/30_RNA速率 - 脂肪和FAP_整合之后")
getwd()

#########step1######################
#ldat <- read.loom.matrices(file = "~/possorted_genome_bam_LYJXT.loom")
ldat1 <- ReadVelocity(file = "E:/BIIF/liutianxia/Adipocyte_scRNA_seq_7/3_结果/18_RNA速率/6609_JJ.loom")
#修改ldat1中的细胞名字。
...
ldat2 <- ReadVelocity(file = "E:/BIIF/liutianxia/Adipocyte_scRNA_seq_7/3_结果/18_RNA速率/6609_SZ.loom")
#修改ldat2中的细胞名字。
...
ldat3 <- ReadVelocity(file = "E:/BIIF/liutianxia/Adipocyte_scRNA_seq_7/3_结果/18_RNA速率/7103_JJ.loom")
#修改ldat3中的细胞名字。
...
ldat4 <- ReadVelocity(file = "E:/BIIF/liutianxia/Adipocyte_scRNA_seq_7/3_结果/18_RNA速率/7103_SZ.loom")
#修改ldat4中的细胞名字。
...
#########step2######################
#为了画一个，前脂肪和脂肪在一起的图。而且他们的亚群，也是跟单独的合并后的亚群保持一致
#这样都是有意义的亚群。所以，方法是，通过细胞的编号和亚群的一一对应的关系。

#先提前把yaqun_1_Adipo_mm, yaqun_1_FAPs_mm弄好
#merge
adipoFAP2 <- merge(yaqun_1_Adipo_mm, yaqun_1_FAPs_mm)
#cluster
adipoFAP2@meta.data$yl2 <- paste(adipoFAP2@meta.data$xibao,
                                 adipoFAP2@meta.data$yl,sep = "_")
Idents(adipoFAP2) <- adipoFAP2@meta.data$yl2
adipoFAP2@active.ident <- factor(adipoFAP2@active.ident, 
                                 levels = c("ASPC_C1","ASPC_C2","ASPC_C3","ASPC_C4","ASPC_C5",
                                            "Adipocyte_C1","Adipocyte_C2","Adipocyte_C3","Adipocyte_C4"))
levels(adipoFAP2)
adipoFAP2@meta.data$rownamesss <- row.names(adipoFAP2@meta.data)

#########step3######################
levels(guolv_2)
yaqun_3 <- subset(x = guolv_2, xibao %in% c("Adipocyte", "ASPC"))
#因为是整合的，去批次的结果，所以必须把DefaultAssay改为integrated，否则会报错。
DefaultAssay(yaqun_3) <- "integrated"
#再细分
#设置随机数
set.seed(123)
#20 .04是原来的
yaqun_3 <- RunUMAP(object = yaqun_3, dims = 1:27)
yaqun_3 <- FindNeighbors(yaqun_3, dims = 1:27)
#备注
yaqun_3 <- FindClusters(yaqun_3, resolution = 0.6)
DefaultAssay(yaqun_3) <- "RNA"
table(yaqun_3$orig.ident)
yaqun_3@meta.data$rownamesss <- row.names(yaqun_3@meta.data)
#分别填入脂肪和前脂肪的亚类的信息
yaqun_3@meta.data$yl2 <- adipoFAP2@meta.data$yl2[match(yaqun_3@meta.data$rownamesss,
                                                       adipoFAP2@meta.data$rownamesss)]

#画
x <- 0.8
plot113 <- DimPlot(yaqun_3,reduction = "umap",label=F, group.by = "yl2", pt.size = x,
                   cols = DiscretePalette(36, palette = "polychrome")[c(4:36)])
ggsave(plot113, file='Plot213-umap-1.pdf', width=5.5, height=4)
plot114 <- DimPlot(yaqun_3,reduction = "umap",label=F, group.by = "yl2", split.by ='orig.ident', pt.size = x,
                   cols = DiscretePalette(36, palette = "polychrome")[c(4:36)])
ggsave(plot114, file='Plot214-umap-2.pdf', width=16, height=4)

#########step4######################
#4个样品分别做
yaqun_1_Adipo_tem <- subset(yaqun_3, orig.ident == "6609_JJ")
#选取
bm_all_no1 <- bm1
#取细胞的子集
...
bm_all_no1_2@meta.data$yl2 <- yaqun_1_Adipo_tem@meta.data$yl2[maichi2]
bm_all_no1_2@meta.data$yl2 <- factor(bm_all_no1_2@meta.data$yl2,
                                     levels = c("ASPC_C1","ASPC_C2","ASPC_C3","ASPC_C4","ASPC_C5",
                                                "Adipocyte_C1","Adipocyte_C2","Adipocyte_C3","Adipocyte_C4"))
#把yl2定为 active.ident
Idents(bm_all_no1_2) <- "yl2"
#速率
...
yaqun_1_Adipo_tem <- subset(yaqun_3, orig.ident == "6609_SZ")
#选取
bm_all_no2 <- bm2
#取细胞的子集
...
bm_all_no2_2@meta.data$yl2 <- yaqun_1_Adipo_tem@meta.data$yl2[maichi2]
bm_all_no2_2@meta.data$yl2 <- factor(bm_all_no2_2@meta.data$yl2,
                                     levels = c("ASPC_C1","ASPC_C2","ASPC_C3","ASPC_C4","ASPC_C5",
                                                "Adipocyte_C1","Adipocyte_C2","Adipocyte_C3","Adipocyte_C4"))
#把yl2定为 active.ident
Idents(bm_all_no2_2) <- "yl2"
#速率
...
yaqun_1_Adipo_tem <- subset(yaqun_3, orig.ident == "7103_JJ")
#选取
bm_all_no3 <- bm3
#取细胞的子集
...
bm_all_no3_2@meta.data$yl2 <- yaqun_1_Adipo_tem@meta.data$yl2[maichi2]
bm_all_no3_2@meta.data$yl2 <- factor(bm_all_no3_2@meta.data$yl2,
                                     levels = c("ASPC_C1","ASPC_C2","ASPC_C3","ASPC_C4","ASPC_C5",
                                                "Adipocyte_C1","Adipocyte_C2","Adipocyte_C3","Adipocyte_C4"))
#把yl2定为 active.ident
Idents(bm_all_no3_2) <- "yl2"
#速率
...
yaqun_1_Adipo_tem <- subset(yaqun_3, orig.ident == "7103_SZ")
#选取
bm_all_no4 <- bm4
#取细胞的子集
...
bm_all_no4_2@meta.data$yl2 <- yaqun_1_Adipo_tem@meta.data$yl2[maichi2]
bm_all_no4_2@meta.data$yl2 <- factor(bm_all_no4_2@meta.data$yl2,
                                    levels = c("ASPC_C1","ASPC_C2","ASPC_C3","ASPC_C4","ASPC_C5",
                                               "Adipocyte_C1","Adipocyte_C2","Adipocyte_C3","Adipocyte_C4"))
#把yl2定为 active.ident
Idents(bm_all_no4_2) <- "yl2"
...
col_3 <- DiscretePalette(36, palette = "polychrome")[c(4:36)]
ident.colors <- col_3
names(x = ident.colors) <- levels(bm_all_no4_2)
